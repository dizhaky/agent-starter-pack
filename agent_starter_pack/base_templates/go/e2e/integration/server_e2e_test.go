...
// serverProcess holds the server process for cleanup
...
func TestA2AErrorHandling(t *testing.T) {
	...
	// This test assumes the server is already running from previous tests
	...
	if err != nil {
		...
		t.Skip("Server not running, skipping error handling test")
	}
	defer resp.Body.Close()
